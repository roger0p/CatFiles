...
function fzf_key_bindings
  ...
  function fzf-cd-widget -d "Change directory"
    ...
    test -n "$FZF_TMUX_HEIGHT"; or set FZF_TMUX_HEIGHT 40%
    begin
      set -lx FZF_DEFAULT_OPTS "--height $FZF_TMUX_HEIGHT --bind=ctrl-z:ignore $FZF_DEFAULT_OPTS $FZF_ALT_C_OPTS"
  ...
  function fzf-z-widget -d "Jump to Directory"
    set -l commandline (__fzf_parse_commandline)
    set -l dir $commandline[1]
    set -l fzf_query $commandline[2]
    set -l prefix $commandline[3]

    set -l FZF_CTRL_J_COMMAND "z -l | sort -rn | cut -c 12-"
    test -n "$FZF_TMUX_HEIGHT"; or set FZF_TMUX_HEIGHT 40%
    begin
      set -lx FZF_DEFAULT_OPTS "--height $FZF_TMUX_HEIGHT --bind=ctrl-z:ignore $FZF_DEFAULT_OPTS $FZF_ALT_C_OPTS"
      eval "$FZF_CTRL_J_COMMAND | "(__fzfcmd)' +m --query "'$fzf_query'"' | read -l result
  ...
  bind \cn fzf-z-widget
  bind \ec fzf-cd-widget

  if bind -M insert > /dev/null 2>&1
    bind -M insert \cf fzf-file-widget
    bind -M insert \ck fzf-history-widget
    bind -M insert \cn fzf-z-widget
